...
final class OnboardingFifthView: UIView {

    //MARK: - Components
    private let titleLabel = StyledLabel(config: .titleSemi20).then {
        $0.numberOfLines = 0
        $0.attributedText = OnboardingFifthView.makeTitle(
            primary: "나의 재고를",
            accent: "편리하게 관리해요"
        )
    }
    ...
    private let descriptionLabel = StyledLabel(config: .body14).then {
        $0.numberOfLines = 0
        $0.attributedText = OnboardingFifthView.makeDescription(
...
//MARK: - Configure UI
private extension OnboardingFifthView {
    func configureUI() {
